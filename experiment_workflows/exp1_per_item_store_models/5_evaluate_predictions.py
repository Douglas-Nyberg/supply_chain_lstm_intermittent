#////////////////////////////////////////////////////////////////////////////////#
# File:         5_evaluate_predictions.py                                        #
...
def load_prediction_files(
    ...
    # Predictions are stored in model-specific subdirectories
    # Structure: predictions_dir/model_type/model_type_point_forecasts.csv
    model_predictions_dir = predictions_dir / model_type
    ...
    if quantile_forecast_file_path.exists():
        try:
            ...
            # Validate quantile predictions format and content
            if not validate_quantile_predictions(quantile_predictions_df, model_type):
                logger.warning(f"  Quantile predictions validation failed for {model_type}, will skip uncertainty metrics")
                quantile_predictions_df = None
            
        except Exception as e:
            logger.error(f"  Failed to load quantile forecasts for {model_type}: {str(e)}")
            quantile_predictions_df = None
    else:
...
def validate_quantile_predictions(df: pd.DataFrame, model_type: str) -> bool:
    """
    Validate quantile predictions format and detect common issues.
    
    Args:
        df: Quantile predictions DataFrame
        model_type: Model name for logging
    ...
    try:
        ...
        if missing_cols:
            logger.warning(f"  Missing required columns in quantile predictions: {missing_cols}")
            return False
        
        # Check for quantile columns (should have pattern like 'q_0.500', 'quantile_0.500', etc.)
        quantile_cols = [col for col in df.columns if any(pattern in col.lower() for pattern in ['q_', 'quantile'])]
        if len(quantile_cols) == 0:
            logger.warning(f"  No quantile columns found in quantile predictions for {model_type}")
            return False
        
        logger.debug(f"  Found {len(quantile_cols)} quantile columns: {quantile_cols[:3]}..." if len(quantile_cols) > 3 else f"  Found quantile columns: {quantile_cols}")
        
        # Check for NaN/inf values in quantile predictions
        ...
        if total_nans > 0:
            logger.warning(f"  Found {total_nans} NaN values in quantile predictions for {model_type}")
            logger.debug(f"    NaN counts by column: {dict(nan_counts[nan_counts > 0])}")
        
        if total_infs > 0:
            logger.warning(f"  Found {total_infs} infinite values in quantile predictions for {model_type}")
            logger.debug(f"    Inf counts by column: {dict(inf_counts[inf_counts > 0])}")
        
        # Check if predictions are all zeros (common failure mode)
        for col in quantile_cols[:5]:  # Check first 5 quantile columns
            if col in df.columns:
                col_values = df[col].values
                if len(col_values) > 0:
                    non_zero_count = np.sum(np.abs(col_values) > 1e-10)
                    if non_zero_count == 0:
                        logger.warning(f"  All predictions are zero in column {col} for {model_type}")
                    elif non_zero_count < len(col_values) * 0.1:  # Less than 10% non-zero
                        logger.debug(f"  Column {col} has only {non_zero_count}/{len(col_values)} non-zero predictions")
        
        # Check prediction ranges for reasonableness (sales should be non-negative)
        for col in quantile_cols[:3]:  # Check first 3 quantile columns
            if col in df.columns:
                col_values = df[col].values
                min_val, max_val = np.min(col_values), np.max(col_values)
                if min_val < -100:  # Significantly negative predictions are suspicious
                    logger.warning(f"  Suspicious negative predictions in {col}: min={min_val:.3f}")
                if max_val > 1000:  # Extremely high predictions might indicate scaling issues
                    logger.debug(f"  High predictions in {col}: max={max_val:.3f}")
        
        return True
        
    except Exception as e:
        logger.error(f"  Error during quantile prediction validation for {model_type}: {str(e)}")
        return False
...
def calculate_all_accuracy_metrics_per_series(
    ...
    """Calculates ALL 14 accuracy metrics for each series."""
    logger.info("Calculating ALL accuracy metrics per series...")
    metric_results_list = []
    ...
    for _, prediction_row in tqdm(point_predictions_df.iterrows(), total=len(point_predictions_df), desc="Comprehensive Accuracy Metrics"):
        ...
        # Removed MAPE and SMAPE as they are fundamentally unsuitable for intermittent/sparse demand patterns
        # Over 200+ series had MAPE values > 1e10 due to near-zero actuals causing division by zero issues
        # Focus on metrics that are meaningful for retail forecasting research        
        ...
        if len(series_train_history) >= config.EVALUATION_CONFIG["seasonality"] + 1:
            # MASE re-enabled after fixing series validation mismatch between LSTM and classical models
            # Added series validation to LSTM training to filter out extremely sparse series
...
def calculate_all_uncertainty_metrics_per_series(
    ...
    """Calculates ALL 11 uncertainty metrics for each series."""
    logger.info("Calculating ALL uncertainty metrics per series...")
    ...
    for base_series_id in tqdm(unique_series, desc="Uncertainty Metrics"):
        ...
        try:
            ...
            if 0.1 in aligned_predictions and 0.9 in aligned_predictions:
                ...
                metrics["interval_score_80"] = np.nan
            ...
            if 0.1 in aligned_predictions and 0.9 in aligned_predictions:
                ...
                metrics["constraint_violation"] = np.nan
...
def main():
    ...
    for model_type in args.models_to_evaluate:
        ...
        # Calculate aggregated metrics for this model using robust aggregation for outlier-prone metrics
        if not model_comprehensive_df.empty:
            # Import robust aggregation functions
            from src.evaluators.robust_aggregation import aggregate_metrics_robust, summarize_outliers
            
            # Use robust aggregation for metrics prone to outliers (especially with intermittent demand)
            # Removed MAPE from sensitive metrics since we no longer calculate it
            aggregated_metrics = aggregate_metrics_robust(
                model_comprehensive_df, 
                sensitive_metrics=['med_ase', 'mase', 'mrae', 'linex'],
                robust_method='trimmed'
            )
            all_model_results[model_type] = aggregated_metrics
            
            # Log outlier summary for MED_ASE if it exists
            if 'med_ase' in model_comprehensive_df.columns:
                outlier_summary = summarize_outliers(model_comprehensive_df, 'med_ase')
                logger.info(f"{model_type} MED_ASE outlier analysis:")
                logger.info(f"  Total series: {outlier_summary['total_count']}")
                logger.info(f"  Outliers detected: {outlier_summary['outlier_count']} ({outlier_summary['outlier_percentage']:.1f}%)")
                logger.info(f"  Mean with outliers: {outlier_summary['mean_with_outliers']:.2e}")
                logger.info(f"  Robust mean (outliers removed): {outlier_summary['mean_without_outliers']:.4f}")
                logger.info(f"  Median: {outlier_summary['median']:.4f}")
                if outlier_summary['outlier_series']:
                    logger.info(f"  Outlier series: {outlier_summary['outlier_series'][:5]}...")  # Show first 5
