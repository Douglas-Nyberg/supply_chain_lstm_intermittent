...
def main():
    ...
    if args.model_type == 'lstm':
        # Load embedding specs from the pickle file (same as training script)
        embedding_pickle_path = Path(args.input_data_dir) / config.DATA_CONFIG["embedding_info_pickle_name"]
        with open(embedding_pickle_path, 'rb') as f:
            embedding_data = pickle.load(f)
        
        if isinstance(embedding_data, dict) and 'feature_specs' in embedding_data:
            # Simplified synthetic data structure
            feature_specs = embedding_data.get('feature_specs', {})
            categorical_features_used = embedding_data.get('categorical_features', [])
            
            global_embedding_specs_for_lstm = {}
            for feat_name in categorical_features_used:
                if feat_name in feature_specs:
                    spec = feature_specs[feat_name]
                    global_embedding_specs_for_lstm[feat_name] = (spec['vocab_size'], spec['embedding_dim'])
            
            # Use EXACT same logic as training script (3_train_lstm_models_mc.py lines 450-457)
            # Categorical features are those with integer dtype and in embedding specs
            cat_feats = [c for c in global_embedding_specs_for_lstm.keys() if c in full_train_df_global.columns and pd.api.types.is_integer_dtype(full_train_df_global[c].dtype)]
            lstm_categorical_cols = cat_feats
            
            # Numerical features are numeric columns excluding ID, date, target, and categorical
            num_feats = [c for c in full_train_df_global.columns if c not in (['id','date','sales'] + cat_feats) and pd.api.types.is_numeric_dtype(full_train_df_global[c])]
            lstm_numerical_cols = num_feats
        else:
            # Fallback for original M5 structure
