...
#!/usr/bin/env python3
"""
Master experiment runner for Monte Carlo experiment.
...
def main():
    """Main entry point to run the MC experiment."""
    # Parse command-line arguments
    parser = argparse.ArgumentParser(description="Run Monte Carlo Forecasting Experiment.")
    parser.add_argument(
        "--models-to-run", nargs='+',
    ...
    parser.add_argument("--steps", type=str, default="all", help="Which steps to run (all, preprocessing, training, prediction, evaluation).")
    parser.add_argument("--limit-series", type=int, default=None, help="Limit number of series to process (useful for testing).")
    
    args = parser.parse_args()

    run_specific_experiment_name = f"{config.EXPERIMENT_TAG}_{args.run_tag}"
    logger.info("=" * 70)
    logger.info(f"STARTING MC EXPERIMENT RUN: {run_specific_experiment_name}")
    ...
    # Handle --steps argument
    if args.steps != "all":
        if args.steps == "preprocessing":
            args.skip_training = args.skip_prediction = args.skip_evaluation = args.skip_visualization = args.skip_statistical_comparison = True
        elif args.steps == "training":
            args.skip_preprocessing = args.skip_prediction = args.skip_evaluation = args.skip_visualization = args.skip_statistical_comparison = True
        elif args.steps == "prediction":
            args.skip_preprocessing = args.skip_training = args.skip_evaluation = args.skip_visualization = args.skip_statistical_comparison = True
        elif args.steps == "evaluation":
            args.skip_preprocessing = args.skip_training = args.skip_prediction = args.skip_visualization = args.skip_statistical_comparison = True
    ...
    if not args.skip_preprocessing:
        ...
        if not run_script_step("1_preprocess_exp_mc.py", preprocess_args):
    ...
    if not args.skip_training:
        ...
        if classical_to_train:
            ...
            if not run_script_step("2_train_classical_models_mc.py", train_classical_args):
        ...
        if lstm_to_train: 
            ...
            if args.limit_series is not None:
                train_lstm_args.extend(["--limit-series", str(args.limit_series)])
            elif config.DATA_CONFIG["limit_items"] is not None:
                train_lstm_args.extend(["--limit-series", str(config.DATA_CONFIG["limit_items"])])
            # Force CPU to avoid CUDA issues
            train_lstm_args.extend(["--device", "cpu"])

            if not run_script_step("3_train_lstm_models_mc.py", train_lstm_args):
    ...
    if not args.skip_prediction:
        ...
        for model_type in args.models_to_run:
            ...
            if not run_script_step("4_predict_models_mc.py", predict_args):
                overall_success = False
    ...
    if not args.skip_evaluation:
        ...
        if not run_script_step("5_evaluate_predictions_mc.py", eval_args):
            overall_success = False
    ...
    if not args.skip_visualization:
        ...
        if not run_script_step("7_visualize_results_mc.py", viz_args):
            overall_success = False
    ...
    if not args.skip_statistical_comparison and len(args.models_to_run) > 1:
        ...
        if not run_script_step("6_statistical_comparison_mc.py", comparison_args):
            overall_success = False
    ...
    logger.info("=" * 70)
    if overall_success:
        logger.info(f"MC EXPERIMENT RUN '{run_specific_experiment_name}' COMPLETED SUCCESSFULLY.")
    else:
        logger.warning(f"MC EXPERIMENT RUN '{run_specific_experiment_name}' COMPLETED WITH ONE OR MORE FAILURES.")
